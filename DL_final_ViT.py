...
from torch.autograd import Variable
model = torch.hub.load('pytorch/vision:v0.14.0', 'vit_b_16', weights=True)
...
weight1 = model.conv_proj.weight.clone()
new_first_layer  = nn.Conv2d(4, 768, kernel_size=(16, 16), stride=(16, 16)).requires_grad_()
new_first_layer.weight[:,:3,:,:].data[...] =  Variable(weight1, requires_grad=True)
model.conv_proj = new_first_layer

#check model weight
print(model.conv_proj.weight.size())
...
classifier_input = model.heads.head.in_features
